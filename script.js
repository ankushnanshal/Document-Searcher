...
if (confirmDeleteBtn) {
  confirmDeleteBtn.addEventListener('click', function() {
    if (pendingDeleteData) {
      const { subject, index } = pendingDeleteData;
      deleteAnnouncement(subject, index);
      closeDeleteConfirmModal();
    }
  });
}
  ...
  modal.innerHTML = `
        <div class="modal-glass-card" style="max-width: 500px;">
            <div class="modal-top-bar">
                <h3>${cardData.subject}</h3>
                <button class="modal-dismiss-icon" onclick="this.closest('.modal-overlay').remove()">&times;</button>
            </div>
            <div style="display: flex; flex-direction: column; gap: 12px; padding: 8px 0;">
                <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border-glass);">
                    ...
                    <span style="font-weight: 500;">${cardData.branch}</span>
                </div>
                <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border-glass);">
                    <span style="color: var(--text-secondary);">Semester</span>
                    <span style="font-weight: 500;">Semester ${cardData.semester}</span>
                </div>
                <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border-glass);">
                    <span style="color: var(--text-secondary);">Teacher</span>
                    <span style="font-weight: 500;">${cardData.teacher}</span>
                </div>
                <div style="display: flex; justify-content: space-between; padding: 8px 0;">
                    <span style="color: var(--text-secondary);">Status</span>
                    <span style="font-weight: 500; color: #4ade80;">Active</span>
                </div>
            </div>
...
function deleteAcademicCard(subject) {
  if (activeUserEmail !== 'ankushadmin@gmail.com') {
  ...
  confirmModal.innerHTML = `
        <div class="modal-glass-card" style="max-width: 400px;">
            <div class="modal-top-bar">
                <h3 style="color: #ef4444;">Delete Academic Resource</h3>
                <button class="modal-dismiss-icon" onclick="this.closest('.modal-overlay').remove()">&times;</button>
            </div>
  ...
  if (isUpper) {
    options = [
      { label: '👁️ View', action: () => { viewAcademicCard(cardData);
          menu.remove(); } },
      { label: '✏️ Edit', action: () => { editAcademicCard(cardData, index);
          menu.remove(); } },
      { label: '📌 Pin', action: () => { pinAcademicCard(index);
          menu.remove(); } },
    ];
    if (isAdmin) {
      options.push({ label: '🗑️ Delete', action: () => { deleteAcademicCard(cardData.subject);
          menu.remove(); } });
    }
  } else {
    options = [
      ...
      { label: '📌 Pin', action: () => { pinAcademicCard(index);
          menu.remove(); } },
      { label: '📂 Move', action: () => { moveAcademicCard(index);
          menu.remove(); } },
      { label: '🔗 Share', action: () => { shareAcademicCard(cardData);
          menu.remove(); } }
    ];
  }
  options.forEach(opt => {
...
function postAnnouncement() {
  ...
  if (!text && !window.pendingAttachments) {
